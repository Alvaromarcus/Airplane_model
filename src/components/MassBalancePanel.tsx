...
const selectCls = 'w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-md text-xs bg-white dark:bg-gray-800 text-gray-900 dark:text-white';

export default function MassBalancePanel({ balance, settings, onChange, layout }: Props) {
  const { t } = useTranslation();
  if (!balance) return <div className="p-4 text-sm text-gray-500">—</div>;
  const b = balance;
  ...
  return (
    <div className="p-4 space-y-4 text-gray-800 dark:text-gray-200 text-sm">
      <div className="flex items-center gap-2 pb-1 border-b border-gray-200 dark:border-gray-700">
        ...
        <h3 className="text-sm font-bold uppercase tracking-wider text-gray-900 dark:text-gray-100">{t('mb_title')}</h3>
      </div>

      {/* Component choices */}
      <div className="grid grid-cols-2 gap-2">
        <label className="text-xs">
          <span className="block text-gray-500 dark:text-gray-400 mb-1">{t('mb_servos')}</span>
        ...
        <label className="text-xs">
          <span className="block text-gray-500 dark:text-gray-400 mb-1">{t('mb_battery')}</span>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 p-2">
          <p className="text-gray-500 dark:text-gray-400">{t('mb_auw')}</p>
          <p className="text-lg font-semibold text-gray-900 dark:text-white">{Math.round(b.auw)} g</p>
          <p className="text-gray-400">{b.wingLoading.toFixed(1)} g/dm²</p>
        </div>
        <div className="rounded border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-2">
          ...
          <p className="text-lg font-semibold text-gray-900 dark:text-white">{Math.round(bat.s - bat.size![2] / 2)} mm</p>
          <p className="text-gray-500 dark:text-gray-400 leading-tight">{t('mb_battery_front')} {refLabel}</p>
        </div>
      </div>
      ...
      {/* Mass table */}
      <div>
        <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">{t('mb_breakdown')}</div>
        <table className="w-full text-xs">
          <tbody>
            {rows.map(r => (
              <tr key={r.key} className="border-b border-gray-100 dark:border-gray-700/60">
                <td className="py-1 pr-1">
                  <span className="inline-flex items-center gap-1.5">
                    {'color' in r && r.color && <i className="inline-block w-2.5 h-2.5 rounded-sm" style={{ background: r.color }} />}
                    {r.label}
                  </span>
                  {r.detail && <span className="block text-[10px] text-gray-400">{r.detail}</span>}
                </td>
                <td className="py-1 text-right tabular-nums">{r.mass.toFixed(0)} g</td>
                <td className="py-1 pl-2 text-right tabular-nums text-gray-400 w-10">{((r.mass / b.auw) * 100).toFixed(0)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {b.linkages.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">{t('mb_pushrods')}</div>
      ...
      <p className="text-[11px] leading-snug text-gray-400">{t('mb_note')}</p>
    </div>
  );
}
